...
以多週期（月/週/日/4H/60m/15m/5m/1m）聯動的結構推演工具，整合「7 字訣」「門控（W/M）」「波動標籤（1/2/3/A/B/C）」與能量加權，提供趨勢評分、情境預言、三層防禦聯動與極端行情警示。此為前端單頁版（HTML+CSS+JS），適合教學、策略設計與視覺化演練。
...
- 多空評分
  - 1/3/B 視為 +1，2/A/C 視為 -1；若是 B 且 7 字訣為 DOWN，視為 -1
  - 各層級方向 × 權重相加：> 20 多頭慣性；< -20 空頭重力；介於其間為中性整理
- 狀態推演
  - 依起始標籤與 ratio 對步數做輪替，生成各時框標籤序列
- 極端警示
...
- 快速使用：
  1. 將你提供的 HTML 貼入並另存為 `index.html`
  2. 直接以瀏覽器開啟，或部署到任一靜態伺服器（Nginx/Apache/Vercel/Netlify/Cloudflare Pages）
- HTTPS 建議：
  - 對外服務時建議使用反向代理（Nginx）加 Let’s Encrypt 憑證，強制 HTTPS
- PWA 提示：
...
- 情境規則：擴充 Scenario 條件與極端警示邏輯（更多宏觀/微觀組合）
- 視覺樣式：在 `<style>` 中調整主題色、字級、標籤色階、卡片樣式
- 門控演算法：以更完整的破門/偽突破/回抽測門邏輯替換目前的簡化輪替
...
- 趨勢評分如何計算？
  - 各層級標籤方向 × 權重相加（B 段受 7 字訣修正），> 20 多頭慣性、< -20 空頭重力、介於其間為中性
- 門控提示的來源？
